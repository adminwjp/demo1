...
namespace Product.Api.Manager
{
    ...
    public class SpecController : BaseController
    {
        ...
        protected ISpecAppService ApiService { get; set; }
        public SpecController(ISpecAppService service, IMediator mediator)
        {
        ...
        [HttpPost("Add")]
        public virtual async Task<ResponseApi> Add([FromForm, FromBody] CreateSpecInput obj)
        {
        ...
        public virtual async System.Threading.Tasks.Task<ResponseApi> Sotck([FromForm, FromBody] NumberChanngeCommand number)
        {
            var result = await mediator.Send(number);
            return result ? ResponseApi.OkByEnglish : ResponseApi.FailByEnglish;
        }
        ...
        ///<return>返回修改结果,大于0 返回修改成功,小于等于0 返回修改失败 </return>
        [HttpPost("Modify")]
        public virtual async System.Threading.Tasks.Task<ResponseApi> Modify([FromForm, FromBody] UpdateSpecInput obj)
        {
        ...
        public virtual async System.Threading.Tasks.Task<ResponseApi<ResultDto<GetSpecOutput>>> GetResultByPage([FromForm, FromBody] GetSpecInput obj, int page, int size)
        {
            var data = await ApiService.FindListByPageAsync(obj, page, size);
            var count = await ApiService.CountAsync(obj);
            var res = new ResultDto<GetSpecOutput>(data, page, size, count);
            return ResponseApi<ResultDto<GetSpecOutput>>.CreateSuccess().SetData(res);
        }
