...
namespace Tunynet.Common.Repositories
{
    ...
    /// 标签仓储的具体实现类
    /// </summary>
    public class TagRepository<T> : BaseRepository<T>, ITagRepository<T> where T : Tag
    {
        /// <summary>
        /// 默认Database实例
        /// </summary>
        ...
        {
            Sql sql = Sql.Builder;
            sql.Append("select * from tn_Tags where TagName = @0 and TenantTypeId = @1", entity.TagName, entity.TenantTypeId);
        ...
        public void DeleteTrashDatas()
        {
            ...
            foreach (var tenantType in tenantTypes)
            {
                ...
                sqls.Add(Sql.Builder.Append("delete from tn_ItemsInTags")
                                    .Where("not exists (select 1  from " + pd.TableName + " where tn_ItemsInTags.ItemId = " + pd.TableName + "." + pd.PrimaryKey + ") and tn_ItemsInTags.TenantTypeId = @0"
