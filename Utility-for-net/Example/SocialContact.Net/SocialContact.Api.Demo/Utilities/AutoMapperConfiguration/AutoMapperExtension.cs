...
namespace Tunynet.Common
{
    public static class AutoMapperExtension
    {
        ...
        public static List<TDestination> MapToList<TSource, TDestination>(this IEnumerable<TSource> source)
        {
            ...
            //获取所有映射类型
            var allConfiguration = AutoMapperObjectMapper.Empty.Mapper.ConfigurationProvider.GetAllTypeMaps();
            //判断当前映射类型是否已经初始化过了
            ...
            if (configurationCount > 0)
            {
                foreach (var item in source)
                    TDestinations.Add(AutoMapperObjectMapper.Empty.Map<TSource, TDestination>(item));
            ...
            where TDestination : class
        {
            ...
            var configurationCount = allConfiguration.Where(n => n.SourceType.Name == typeof(TSource).Name && n.DestinationType.Name == typeof(TDestination).Name).Count();
            if (configurationCount > 0)
                return AutoMapperObjectMapper.Empty.Map(source, destination);
            var config = new MapperConfiguration(cfg =>
             {
                 cfg.CreateMap<TSource, TDestination>();
             });
