...
type consulServiceRegistry struct {
	serviceInstances     map[string]map[string]ServiceInfo
	client               api.Client
	localServiceInstance ServiceInfo
}

func (c consulServiceRegistry) Register(serviceInstance ServiceInfo) bool {
	// 创建注册到consul的服务到
	...
	if c.serviceInstances == nil {
		c.serviceInstances = map[string]map[string]ServiceInfo{}
	}
...
type DiscoveryClient interface {
	 ...
	 */
	GetInstances(serviceId string) ([]ServiceInfo, error)
...
func (c consulDiscoveryClient) GetInstances(serviceId string) ([]ServiceInfo, error) {
	catalogService, _, _ := c.client.Catalog().Service(serviceId, "", nil)
	if len(catalogService) > 0 {
		result := make([]ServiceInfo, len(catalogService))
