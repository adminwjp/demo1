...
public class InitialLetterLowerCaseUpperSqlUbterceptor extends  AbstractSqlInterceptor {
    @Override
    protected SqlSource getSqlSource(SqlSource sqlSource) {
        SqlSourceWrapper source= new SqlSourceWrapper(sqlSource);
        source.setStringFormat(StringUtil.StringFormat.InitialLetterLowerCaseUpper);
        return source;
    }
}