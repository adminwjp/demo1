...
public class InitialLetterUpperCaseLowerSqlInterceptor extends  AbstractSqlInterceptor {

    @Override
    protected SqlSource getSqlSource(SqlSource sqlSource) {
        SqlSourceWrapper source= new SqlSourceWrapper(sqlSource);
        source.setStringFormat(StringUtil.StringFormat.InitialLetterUpperCaseLower);
        return source;
    }
}